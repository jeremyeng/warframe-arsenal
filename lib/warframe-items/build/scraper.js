...
class Scraper {
  ...
  addCategory (item, type) {
    switch (type) {
      ...
      case 'Weapons':
        if (item.slot === 5 && !item.name.includes('Zaw')) item.category = 'Melee'
  ...
  addDropRate (item) {
    ...
    if (!dropChancesChanged) {
      if (item.components) {
        for (let component of item.components) {
          let savedDrops
          const matchingItem = precompiled.find(i => i.name === item.name)
          if (matchingItem) {
            const components = matchingItem.components
